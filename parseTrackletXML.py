...
class Tracklet(object):
    ...
    # def __iter__(self):
    #     return (x for x in range(self.nFrames))    
    ...
    def __getitem__(self, idx):
        ...
        """
        # while self.cur < self.nFrames:
        #     yield self.cur
        #     self.cur += 1    
        if self.amtOccs is None:
            return self.trans[idx], self.rots[idx], self.states[idx], self.occs[idx], self.truncs[idx], \
                None, None, self.firstFrame + idx
        else:
            return self.trans[idx], self.rots[idx], self.states[idx], self.occs[idx], self.truncs[idx], \
                self.amtOccs[idx], self.amtBorders[idx], self.firstFrame + idx
#end: class Tracklet
